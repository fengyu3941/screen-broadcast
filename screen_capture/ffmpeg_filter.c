    ...
    if (codec && codec->sample_fmts) {
        ...
        if (*p == -1) {
            if((codec->capabilities & CODEC_CAP_LOSSLESS) && av_get_sample_fmt_name(st->codec->sample_fmt) > av_get_sample_fmt_name(codec->sample_fmts[0]))
                writeErrorMsg( "Conversion will not be lossless.\n");
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        writeErrorMsg( "Only video and audio filters supported "
               "currently.\n");
        exit_program(1);
    }
    ...
    if (in->name) {
        ...
        if (file_idx < 0 || file_idx >= nb_input_files) {
            writeErrorMsg( "Invalid file index %d in filtergraph description %s.\n",
        ...
        if (!st) {
            writeErrorMsg( "Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.\n", p, fg->graph_desc);
            exit_program(1);
        }
        ...
        if (i == nb_input_streams) {
            writeErrorMsg( "Cannot find a matching stream for "
                       ...
                       const char *filter_name)
{
    ...
    if (!trim) {
        writeErrorMsg( "%s filter not present, cannot limit "
               "recording time.\n", name);
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
        writeErrorMsg( "Cannot connect video filter to audio input\n");
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type != AVMEDIA_TYPE_AUDIO) {
        writeErrorMsg( "Cannot connect audio filter to non audio input\n");
        return AVERROR(EINVAL);
    }
...
int configure_filtergraph(FilterGraph *fg)
{
    ...
    if (simple && (!inputs || inputs->next || !outputs || outputs->next)) {
        writeErrorMsg( "Simple filtergraph '%s' does not have "
